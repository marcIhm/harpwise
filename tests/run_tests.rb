...
raise 'Cannot run as a snap' if ENV['SNAP_NAME']
...
raise 'Could not parse term size from libexec/config.rb' unless $term_min_width && $term_min_height
...
raise "These programs are needed but cannot be found: \n  #{not_found.join("\n  ")}\nyou may need to install them" if not_found.length > 0
...
known_not = ['harpwise tools transcribe wade.mp3', 'harpwise licks a -t starred']
...
usage_types.values.map {|p| p[0]}.each do |fname|
  File.read("docs/_txt/#{fname}.txt").lines.map(&:strip).each do |l|
    ...
    # lines starting with '| harpwise' are code examples generated by org
    next unless l.start_with?('| harpwise ')

    l.gsub!('\\', '')
    # ignore known false positives
    if known_not.all? {|kn| !l[kn]}
      usage_examples << l[2..-1]
...
num_exp = 121
raise "Unexpected number of examples #{usage_examples.length} instead of #{num_exp}\n" unless usage_examples.length == num_exp
...
do_test 'id-23: print list of licks with tags' do
  ...
  ["  wade ..... fav,favorites,samples,has-rec,shifts-five,shifts-flat-seventh,mostly-blues\n",
   "  st-louis ..... favorites,samples,has-rec,shifts-five,shifts-flat-seventh,mostly-mape\n",
   ...
   "  box1-v ..... box,box1,v-chord,no-rec,shifts-four,shifts-five,shifts-eight,mostly-chord-v,mostly-blues,mostly-mape\n",
   ...
   "  box2-v ..... box,box2,v-chord,no-rec,shifts-four,shifts-five,shifts-eight,mostly-chord-v,mostly-blues,mostly-mape\n",
...
do_test 'id-37c: change option --tags with cursor keys' do
  ...
  sleep 8
  tms 'q'
  expect { screen[22]['Lick Name: special'] }
  wait_for_end_of_harpwise
  dump = read_testing_dump('end')
...
do_test 'id-46a: verify persistent tag "starred"' do
  ...
  tms :ENTER
  wait_for_end_of_harpwise
  expect { screen[9]['wade ..... fav,favorites,samples,unstarred,has-rec'] }
  kill_session
end
...
do_test 'id-50a: tools keys' do
  ...
  tms :ENTER
  expect { screen[10] == 'Colors: row for harp-key B in green' }
...
do_test 'id-63a: print and calculate all intervals' do
  new_session
  tms 'harpwise tools all-inters e5'
  tms :ENTER
  sleep 2
  expect { screen[12]['flat Seventh:   -8      d6'] }
  kill_session
end
...
do_test 'id-86c: print jams' do
  ...
  tms :ENTER
  sleep 2
  expect { screen[9]['fancy_jamming'] }
  expect { screen[9]['c,g; box1; 12bar; unknown'] }
  expect { !screen[9]['.json'] }
  expect { screen[10]['with_variations'] }
  expect { screen[11]['box2'] }
...
do_test 'id-96b: quiz-flavour match-scale' do
  ...
  tms :ENTER
  expect { screen[6]['mipe-middle:   -2  -3/  +4  -4  -5  +6'] }
  kill_session
end
...
do_test 'id-98: loop via signal' do
  ...
  tms :ENTER
  sleep 2
  pid = `ps -ef`.lines.find {|l| l['harpwise'] && l['ruby']}.split[1]
...
do_test 'id-114: play licks next and previous' do
  ...
  sleep 6
  txt = 'Lick   st-louis'
  expect { screen[16][txt] || screen[16][txt] }
  tms :ENTER
  sleep 6
...
do_test 'id-117: check errors for bogus lickfiles' do
  ...
  Dir[Dir.pwd + '/tests/data/bad_lickfiles/*'].each do |file|
    msg = file2err[File.basename(file)] || raise("Unknown bad lickfile #{file}")
    new_session
...
do_test 'id-118: read and check a fancy lickfile' do
  ...
  expect(dump[:licks][2]) { dump[:licks][2][:tags] == %w[five four no-rec shifts-four shifts-five shifts-flat-seventh shifts-eight mostly-chord-iv mostly-blues mostly-mape] }
...
do_test 'id-133a: check content of man-page' do
  new_session
  ...
  man_cmd = "MANPATH=#{mandir}/../ man harpwise | cat"
  tms man_cmd
  tms :ENTER
  expect(man_cmd) {screen[19]['SEE ALSO']}
  man_cmd_err_only = "man --warnings -E UTF-8 -l -Tutf8 -Z -l #{$installdir}/man/harpwise.1 >/dev/null"
  man_err = sys(man_cmd_err_only)
  raise "Processing man-page via '#{man_cmd_err_only}' produces this error: #{man_err}" if man_err != ''
...
do_test 'id-135: use harpwise jamming and listen as advised by its usage' do
  ...
  raise 'Did not find suggested command for harpwise in output' unless usg_cmd_hw

  usg_cmd_hw.tr!('|', '').strip!
  usg_cmd_jam = lines.find {|l| l['harpwise jam along 12bar']}
  raise 'Did not find suggested command for jamming in output' unless usg_cmd_jam

  usg_cmd_jam.tr!('|', '').strip!
...
do_test 'id-136: harpwise jamming list' do
  ...
  tms :ENTER
  wait_for_end_of_harpwise
  expect { screen[14]['12bar        #  c,g; box1 +1 Var; 12bar; yesterday + 1 more']}
  expect { screen[15].strip == 'box2' }
  ...
  tms :ENTER
  expect { screen[11]['1:  harpwise listen --scale-prog 12bar --lick-prog box1 --jamming']}
  expect { screen[33]['Notes:   (from']}
  kill_session
end
...
do_test 'id-137a: harpwise jamming note' do
  ...
  tms 'harpwise jam ls'
  tms :ENTER
  expect { screen[15]['12bar        #  c,g; box1 +1 Var; 12bar; unknown']}
...
do_test 'id-144: check consistent usage of short and long description' do
  ...
  sd_man = nil
  ld_man = []
  system('man -l man/harpwise.1 >/tmp/harpwise.man')
  after = nil
  File.read('/tmp/harpwise.man').lines.map(&:strip).each do |line|
    if line == ''
      after = nil
      break if ld_man.length > 0
    else
      ld_man << line if after == 'DESCRIPTION'
      sd_man ||= line if after == 'NAME'
    end
...
do_test 'id-160: jam with variations' do
  ...
  tms :ENTER
  wait_for_end_of_harpwise
  expect { screen[16]['box1 +1 Var'] }
  expect { screen[17]['box2'] }
  kill_session
end
