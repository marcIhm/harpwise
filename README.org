...
* Installation
  ...
  environment is needed. Harpwise is written in ruby and it runs directly
  from within the directory, which you need to clone from github
  first. Vice versa it can simply be removed by deleting the cloned
  directory.

  harpwise has a bunch of dependencies (ruby, aubiopitch, sox, gnuplot)
