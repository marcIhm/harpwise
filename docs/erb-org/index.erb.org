...
* Invoking harpwise
  ...
  terminal should have black (dark) background to match the colors used by
  harpwise. Your terminal should use a fixed-width font.
  
  Please note, that harpwise tries to adapt itself to different screen
  (terminal) sizes. If change the size of your terminal-window (assuming
  that you have a grafical desktop), then harpwise will react accordingly
  and you do not need to restart it.

  You probaby get the best experience if you use a larger font and a
  terminal with a modest size, e.g. 90 cloumns and 30 rows.
...
* Getting started with harpwise
  ...
  <%= cleanup_for_html(IO.read("../_org/usage.org")) %>
...
** Listen
  ...
  [[../images/harpwise-listen.gif]]
   ...
   <%= cleanup_for_html(IO.read("../_org/usage_listen.org")) %>

** Quiz

  [[../images/harpwise-quiz.gif]]
  
   <%= cleanup_for_html(IO.read("../_org/usage_quiz.org")) %>

** Jamming

   <%= cleanup_for_html(IO.read("../_org/usage_jamming.org")) %>

** Licks

   <%= cleanup_for_html(IO.read("../_org/usage_licks.org")) %>

** Play

   <%= cleanup_for_html(IO.read("../_org/usage_play.org")) %>

** Tools

   <%= cleanup_for_html(IO.read("../_org/usage_tools.org")) %>

** Samples

   <%= cleanup_for_html(IO.read("../_org/usage_samples.org")) %>
                  
* Installation

  The latest version of harpwise is a available as a snap:

  #+begin_src
    sudo snap install harpwise
  #+end_src
  
  this also works quite well for WSL2/Windows.
...
* A closer look at the Screen (mode listen)
  ...
  information about the holes you are playing, including its musical
