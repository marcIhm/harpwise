...
* Screencasts
  ...
  recorded however, so some acoustic imagination is required.
...
The possible scales depend on the chosen type of harmonica:
  ...
  scales for chromatic: all, blues, blues-middle, chord-i, chord-i7, chord-iv, 
    chord-iv7, chord-v, chord-v7, mape, one, two
  scales for richter: all, blues, blues-middle, chord-i, chord-i7, chord-iv, 
    chord-iv7, chord-v, chord-v7, drawbends, major, major-full, 
...
The available 29 flavours are:
    ...
    - all, ran, random:
  [2m    all flavours, one at random[0m
    - scales:
  [2m    flavours related with scales at random[0m
    - mic:
  [2m    flavours, that use the microphone; most of them switch to[0m
  [2m    listen-perspective[0m
    - no-mic:
  [2m    flavours, that do not use the microphone, so that no harp is[0m
  [2m    needed; but these flavours are not necessarily silent and may[0m
  [2m    still play sounds themselves[0m
    - silent:
  [2m    flavours, that can be tried with muted speakers, although some[0m
  [2m    of the help might not be as useful[0m
    - inters:
  [2m    flavours, that revolve around intervals[0m
    - layout:
  [2m    flavours, that deal with the layout (holes to notes) of the harp[0m
    - players:
  [2m    harpwise shows details about a famous player and asks for his or[0m
  [2m    her name[0m
    - hear-scale:
  [2m    harpwise plays the holes of a scale and asks for its name[0m
    - not-in-scale:
  [2m    harpwise picks a scale at random and modifies it: holes are[0m
  [2m    shuffled and one is replaced with a foreign hole; this modified[0m
  [2m    scale is played and you should find the foreign hole. The holes[0m
  [2m    are disguised as h1, h2, and so on[0m
    - match-scale:
  [2m    plays a random sequence of holes, that are either a subset of one[0m
  [2m    or many scales and asks for the name of the single or shortest[0m
  [2m    scale containing all these holes[0m
    - play-scale:
  [2m    gives the name of a scale and asks to play it[0m
    - hear-inter:
  [2m    plays an interval and asks for its name[0m
    - add-inter:
  [2m    gives a hole and an interval and asks for the resulting hole,[0m
  [2m    which is obtained by either adding or subtracting[0m
    - play-inter:
  [2m    names a hole and an interval which resolves to a hole; then asks[0m
  [2m    to play back both holes[0m
    - tell-inter:
  [2m    names two holes and asks for the interval between[0m
    - hear-key:
  [2m    picks a random key and its matching note (from octaves 4 or[0m
  [2m    5); plays a sequence of notes starting from this note; asks for[0m
  [2m    the key[0m
    - hear-hole:
  [2m    picks a random key and a named hole-set; then plays a hole from[0m
  [2m    this set and asks for its name[0m
    - play-shifted:
  [2m    wise computes a sequence of holes, that can be shifted by an[0m
  [2m    interval; it plays the original unshifted sequence and names and[0m
  [2m    plays the first hole of the shifted sequence; then you should play[0m
  [2m    BOTH the unshifted and shifted sequence together[0m
    - key-harp-song:
  [2m    gives a key of harmonica and asks for the matching[0m
  [2m    key of song (2nd position) or vice versa[0m
    - hole-note:
  [2m    the wise picks a harp-key then gives a hole and asks for the note[0m
  [2m    or vice versa[0m
    - hole-note-key:
  [2m    picks (and hides) a new key for the harp at random; then, for a[0m
  [2m    named hole-set (e.g. 'blow'), gives holes and matching notes[0m
  [2m    (which depend on the hidden key of harp); then asks for the key[0m
    - hole-hide-note:
  [2m    picks a new key at random and a named hole-set; then shows the[0m
  [2m    hole-set, but hides one note; then asks for the hidden note[0m
    - hear-hole-set:
  [2m    picks a new key at random and a named hole set; then plays it,[0m
  [2m    holes hidden; then asks for the key and the hole set[0m
    - keep-tempo:
  [2m    plays a number of beats defining a certain tempo; then some[0m
  [2m    silence and then some final beats. You are expected to play along[0m
  [2m    and keep the given tempo throughout; harpwise records your playing[0m
  [2m    and checks, how well you kept the tempo[0m
    - hear-tempo:
  [2m    plays a tempo and asks for its bpm[0m
    - replay:
  [2m    harpwise plays a sequence of holes and asks you to play it back[0m
    - hear-chord:
  [2m    taking the harp key as base, the wise chooses a chord (e.g. IV)[0m
  [2m    then it plays the base note as well as the chord. You are expected[0m
  [2m    to identify the chord. For difficulty HARD there are additional[0m
  [2m    chords and for some chords also variations[0m
    - choose:
  [2m    choose flavour interactively[0m
  [2m    (only as extra-argument on commandline)[0m
...
** Play
  ...
  additional arguments):
    ...
    - licks:
  [2m    all licks selected (e.g. by option -t). Add option '--lick-radio'[0m
  [2m    to make harpwise play licks without interruption (but repeats);[0m
  [2m    as an optional argument you may give the number of repeats[0m
    - pitch:
  [2m    play an interactive, adjustable pitch; this may help to find the[0m
  [2m    key of a song by ear-comparison; the initial key, e.g. 'a4' can be[0m
  [2m    supplied as an optional argument[0m
    - interval, inter:
  [2m    interactive, adjustable interval given as arguments[0m
    - progression, prog:
  [2m    take a base and some semitone diffs, then play it in a loop,[0m
  [2m    allowing e.g. to shift each iteration by some semitones e.g.[0m
  [2m    through a chord-progression of i-iv-v. Moreover you may specify[0m
  [2m    two or more progressions with a dot (.) between; those can then[0m
  [2m    be switched for each iteration.[0m
    - chord:
  [2m    play the given holes or notes simultanously as a chord; also[0m
  [2m    useful for tongue-splits.[0m
    - user:
  [2m    play back (if any), what you (the user) have recorded in mode[0m
  [2m    'licks' before[0m
...
** Samples
  ...
  functions, this gives the following choice of extra arguments:
    ...
    - record:
  [2m    harpwise assists while you play the holes for the given type and[0m
  [2m    key of harmonica. After each recording you may hear back and trim.[0m
  [2m    [0m
  [2m    These samples capture your style of playing and the specific sound[0m
  [2m    of your harmonica as far as possible and brings them into[0m
  [2m    harpwise. This way of creating samples gives more interesting[0m
  [2m    results than 'generate' but requires some work from your side;[0m
  [2m    consider recording the most important keys (or even holes) first[0m
  [2m    and use 'generate' for the others. Even 'generate all', as your[0m
  [2m    recordings would not be overwritten.[0m
  [2m    [0m
  [2m    To redo the recording for a single hole, give it as an argument on[0m
  [2m    the commandline.[0m
  [2m    [0m
  [2m    You may even combine this method of recording hole-samples with[0m
  [2m    'generate', as your recorded samples will be kept and preferred[0m
  [2m    over generated samples unless you explicitly use the extra-command[0m
  [2m    'delete'.[0m
    - generate:
  [2m    as a faster alternative to 'record', you may let harpwise generate[0m
  [2m    samples for each hole.  This gives fast results, that, however[0m
  [2m    lack the typical harmonica sound.  You may try different waveforms[0m
  [2m    (e.g. 'sine') with option --wave (with a default of 'pluck').[0m
  [2m    [0m
  [2m    To generate samples for all keys in one pass, you may give 'all'[0m
  [2m    as an additional argument; otherwise samples are created for the[0m
  [2m    current key only.[0m
  [2m    [0m
  [2m    'generate' never overwrites samples that you play during 'record';[0m
  [2m    use 'delete' for this, if required.[0m
    - check:
  [2m    For the current type and key of harmonica, check out and summarize[0m
  [2m    available samples; especially show, which samples have been[0m
  [2m    recorded and which ones generated. Use 'all' to get a summary for[0m
  [2m    each and every key.[0m
    - delete:
  [2m    For the current key, ask for confirmation to delete your recorded[0m
  [2m    samples, if any. As these recorded samples had precedence over any[0m
  [2m    generated samples, this might be necessary to make harpwise use[0m
  [2m    the generated samples (or maybe suggest to use 'generate' first).[0m
  [2m    [0m
  [2m    This can be useful, if you prefer the (maybe) more precise samples[0m
  [2m    that harpwise generates.[0m
  [2m    [0m
  [2m    By giving the argument 'all' you may delete all samples for all[0m
  [2m    keys.[0m
