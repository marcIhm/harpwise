...
And, less interactive:
  ...
  - print :: Print and annotate the holes or scales or licks given. In
    general this can print anything, that the wise knows about
  - tools :: More than 20 tools for various harmonica needs
...
- scales for richter :: blues, blues-middle, chord-i, chord-i7, chord-iv, 
    ...
    major-pentatonic, mape, mape-middle, mape4, mape5, middle, minor, 
    minor-pentatonic, mipe, mipe-middle, mixolydian
...
** Overview of 'harpwise listen'
  ...
  Switch the comment, to try these extended functions:
  
  - journal :: for a quick transcription of the holes played; use this
    to note down musical ideas
  - warbles :: work on your warbles; speed and accuracy
...
** Overview of 'harpwise quiz'
  ...
  For an overview, here are the available flavours:
    ...
    - hit-from-off:
      practice to hit a hole accurately: put down the harmonica and try
      to play the given hole on the spot, clean and correct.
      
      The hole is taken from the (full or low) hole sets blow and draw;
      or, when invoking this flavour directly from the commandline, you
      may specify the holes to choose from as arguments.
    ...
    - keep-tempo:
      plays and blinks a number of beats defining a certain tempo;
      then some silence and then some final beats (blink only).
      You are expected to play along and keep the given tempo
      throughout; harpwise records your playing and checks, how well
      you kept the tempo
    ...
    - last:
      last flavour (from previous quiz sessions)
...
** Overview of 'harpwise play'
  ...
  additional arguments):
    ...
    - pitch:
      ...
      You may also give a frequency as argument, with or without appended
      unit 'Hz' (e.g. 440 or 880Hz), the pitch starts with the given
      frequency and changes in constant increments.
...
** Overview of 'harpwise tools'
  ...
  The mode 'tools' offers 27 utilities, which can be broadly
  categorized into two groups:

  - Transform the given hole or note-material in various ways
  - Support the operation and configuration of harpwise
  ...
  it can be one of:
    ...
    - change-harp:
      Accept a sequence of holes, convert them into
      notes and move these to another harp with a different
      key, keeping the notes in the process, but (of course)
      changing the holes. This answers the question: Can the
      exact same notes be played on another harp?

    - match-harps:
      Accept a sequence of notes or holes and take those through harps
      of all known keys; check for each key, which of the notes are
      available; try this with different hole sets (with and wo bends);
      print the result as a table. This tool answers the question:
      Can a series of holes or notes be played more easily or at all on
      any other harmonica? Can it be played without bends?
    ...
    - keys:
      ...
      arguments, which control coloured emphasis in the chart.
      The first argument is the key of the harp, usually colored in green;
      use '.' or '-' to skip it dont have it colored. The second argument
      is the key of the song (in blue)
      
      As the positions (columns) are a fifth apart from each other, you
      may also read this chart as a circle of fifths.
      
      Use option --brief to print a table for the most common keys and
      for scond position only.
    ...
    - all-intervals, all-inters:
      print all known intervals against the given note or hole

    - interval, inter:
      calculate and print interval between two notes or holes
...
** Usage by Examples
  ...
  the key of c to g (keeping the notes/pitches constant):
    ...
    #+begin_src 
    harpwise tools change-harp c g -1 +2 -3
    #+end_src
  ...
  Help diagnose sound-problems in your setup:
  
    #+begin_src 
    harpwise tools diag
    #+end_src

  will invoke an interactive assistant to check, if your soundsystem
  (e.g. alsa, pulseaudi) is working as expected.


  Check, which harmonica keys can play a given sequence of holes or
  notes. E.g. if you have some holes involving bends, you may want to
  know, if a harp in another key could play them without:

    #+begin_src 
    harpwise tools match-harps -1 -2/ -3// -3
    #+end_src

  this will take the holes given (key of c), converts them to notes
  and tries to find them on harmonicas of all keys, giving the results
  as an overview list. For the example you may then deduce, that a
  g-harmonica would play this as: -2 -3 -4 +5
...
** The header
   ...
   - Line 2: A summary of the current settings: mode, harmonica-type, key
     and scales
...
** The 'comment'
   ...
   E.g. when in mode 'licks' and harpwise expects you to play a sequence
   of holes, the comment-region shows the sequence of holes that you have
