...
module Licks
  ...
  def read_licks graceful: false, lick_file: nil, use_opt_lick_prog: true
    ...
    ($starred = Hash.new {|h, k| h[k] = 0}).merge! Util::yaml_parse($star_file)
    all_licks = []
    licks = nil
    derived = []
    name2prog = Hash.new
    ...
    (File.readlines(lfile) << '[default]').each_with_index do |line, idx|  ## trigger processing of last lick at end of file
      ...
      # holes = value1 value2 ...
        ...
        section[:holes] = holes.split.map do |hole|
          err("Hole '#{hole}' is not among holes of harp #{$harp_holes} (#{where})") unless Theory::musical_event?(hole) || $harp_holes.include?(hole)
          hole
        end
        err "Lick #{sec_title} key 'holes' is empty (#{where})" unless section[:holes].length > 0
        section[:holes_wo_events] = section[:holes].reject {|h| Theory::musical_event?(h)}
        derived[-1] = '  notes = ' + holes.split.map do |hoe|
          Theory::musical_event?(hoe) ? hoe : $harp[hoe][:note]
        end.join(' ')
      ...
      # notes = value1 value2 ...
        ...
        section[:holes] = notes.split.map do |note|
          err("Note '#{note}' is not among notes of harp #{$harp_notes} (#{where})") unless Theory::musical_event?(note) || $harp_notes.include?(note)
          $note2hole[note]
        end
    ...
    all_licks.each do |lick|
      ...
      h2n[lick[:holes].reject {|h| Theory::musical_event?(h)}] << lick[:name]
    end
    ...
    if $opts[:lick_prog] && use_opt_lick_prog
      ...
      [['--tags-all', keep_all],
       ...
       ['--drop-tags-any', drop_any]].each do |opt, tags|
        ...
        Text::print_in_columns tags_licks.to_a.sort, pad: :tabs
  ...
  def find_lick_by_name name
    ...
    Text::print_in_columns($licks.map {|l| l[:name]}.sort, indent: 4, pad: :tabs)
    puts
    if $licks == $all_licks
