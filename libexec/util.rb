...
module Util
  ...
  def display_kb_help what, scroll_allowed, body, wait_for_key: true
    ...
    unless scroll_allowed
      Interact::clear_area_comment
      Interact::ctl_response 'continue'
    end
    print "\e[0m"
  end
  ...
  def shortcut2history_record short
    ...
    $all_licks, $licks, $all_lick_progs = Licks::read_licks unless $licks
  ...
  def switch_modes
    ...
    if $mode_switches == 1
      ...
      if $mode == :listen && %i[quiz licks].include?(mode_prev)
        ...
        $opts[:iterate] = :random
        $opts[:tags_any] = 'journal' if $journal.select {|h| !Theory::musical_event?(h)}.length > 0
      else
    ...
    Text::do_figlet_unwrapped "> > >   #{$mode}", 'smblock'
    tag = "switch to #{$mode}"
  ...
  def recognize_among val, choices, licks: $licks
    ...
    choices.each do |choice|
      ...
      if choice == :hole
        ...
        return choice if Theory::musical_event?(val)
      elsif choice == :scale
  ...
  def print_amongs *choices, **kws
    ...
    choices.flatten.each do |choice|
      ...
      when :hole
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $harp_holes, indent: 4, pad: :tabs
      when :note
        puts "\n- #{adc[0]}"
        puts "    #{adc[1]}:"
      ...
      when :scale
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $all_scales, indent: 4, pad: :tabs, highlight: summary[:highlight]
      when :scale_prog
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $all_scale_progs.keys.sort, indent: 4, pad: :tabs, highlight: summary[:highlight]
      when :extra
        puts "\n- #{adc[0]}:"
        puts get_extra_desc_all(mode: $mode, highlight: summary[:highlight]).join("\n")
      when :inter
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $intervals_inv.keys.reject {_1[' ']}, indent: 4, pad: :tabs, highlight: summary[:highlight]
      when :lick
        all_lnames = $licks.map {|l| l[:name]}
        puts "\n- #{adc[0]}:"
        Text::print_in_columns all_lnames.sort, indent: 4, pad: :tabs, highlight: summary[:highlight]
        if $licks == $all_licks
          puts '  , where set of licks has not been restricted by tags'
        else
          puts "  , where lick selection is done with these options: #{Licks::desc_lick_select_opts(indent: '  ')}"
        end
      when :lick_prog
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $all_lick_progs.keys.sort, indent: 4, pad: :tabs, highlight: summary[:highlight]
      when :last
        puts "\n- #{adc[0]}"
        puts "    #{adc[1]}:"
      when :jam
        puts "\n- #{adc[0]}:"
        Text::print_in_columns $jamming_rel2abs.keys.sort, indent: 4, pad: :long_tabs, highlight: summary[:highlight]
      else
  ...
  def get_extra_desc_all mode: $mode, highlight: nil, for_usage: false, exclude_meta: false
    ...
    $extras_joined_to_desc[mode].each do |k, v|
      ...
      k = Text::highlight_helper(k, highlight)
      lines << indent + "  - #{k}:"
...
def err text
  ...
  Util::puts_context_sources if $opts && $opts[:debug]
  puts
...
class FamousPlayers
  ...
  def show_picture file, name, in_loop, txt_lines, txt_width
    ...
    when 'window'
      ...
      Util::sys('xwininfo -root')
      sw = Util::sys('xwininfo -root').lines.find {|l| l['Width']}.scan(/\d+/)[0].to_i
      pw, ph = Util::sys("feh -l #{file}").lines[1].split.slice(2, 2).map(&:to_i)
    ...
    when 'char'
      ...
      puts "\e[2m  #{file}\e[0m"
      puts Util::sys("chafa -f symbols #{file}")
    ...
    when 'pixel'
      ...
      if ENV['TERM']['kitty']
        ...
        if cwidth_term > txt_width * 1.25
          ...
          print "\e[s"
          sleep 2
          puts Util::sys("kitty +kitten icat --stdin=no --scale-up --z-index -1 --place #{cwidth_term - txt_width}x#{cheight_term}@#{txt_width}x#{cheight_term - txt_lines} --align right #{file}")
          print "\e[u"
        else
          # not enough room, place image below text
          (cheight_term / 2).times {puts}
          print "\e[s"
          puts Util::sys("kitty +kitten icat --stdin=no --scale-up --z-index -1 --place #{cwidth_term}x#{cheight_term / 2}@0x#{cheight_term / 2} --align left #{file}")
          print "\e[u"
        end
        ...
        if cwidth_term > txt_width * 1.25
          ...
          puts Util::sys("img2sixel --width #{pwidth_img} #{file}")
          print "\e[u"
          Interact::sane_term
        else
          # not enough room, place image below text
          puts Util::sys("img2sixel --height #{pheight_cell.to_i * cheight_term / 2} #{file}")
          Interact::sane_term
        end
      end
    else
...
# prepare byebug
def dbg
  Interact::make_term_cooked if $opts
  Kernel.print "\e[0m"
  require 'byebug'
  byebug
end
