...
module ModeLicks
  ...
  def do_licks_or_quiz quiz_scale_name: nil, quiz_holes_inter: nil, quiz_holes_shift_info: nil, quiz_hole_to_hit: nil, lambda_quiz_hint: nil, to_handle: []
    if to_handle && to_handle.length > 0
      ...
      $licks = $all_licks
      holes_or_notes, _, lnames, = Util::partition_for_mode_or_amongs(to_handle, extra_allowed: false)
    ...
    loop do   # forever until ctrl-c, sequence after sequence
      ...
      if to_play[:lick_idx] && Licks::refresh_licks
        to_play.set_lick_and_others_from_idx
        Interact::ctl_response 'Refreshed licks'
      end
      ...
      # nothing to do here, (re)playing will happen further down
        ...
        if $mode == :quiz
          ...
          if start_with
            ...
            if record = Util::shortcut2history_record(start_with)
              to_play.set_lick_idx(record[:lick_idx])
            else
              to_play.set_lick_idx(Licks::find_lick_by_name(start_with))
            end
            # consumed
            start_with = nil
      ...
      if ( !quiz_scale_name && !quiz_holes_inter && !quiz_hole_to_hit && !zero_partial?) ||
        ...
        Util::print_mission('Listen ... and!') unless oride_l_message2
        sleep 0.3
        $ctl_mic[:redraw_mission] = true
      ...
      if oride_l_message2
        Interact::clear_screen_and_scrollback
        system('clear')
      else
        print "\e[#{$lines[:hint_or_message]}H\e[K"
        print "\e[#{$lines[:message2]}H\e[K"
      ...
      begin   # while looping over the same sequence again and again
        ...
        if $ulrec.active?
          ...
          if $ulrec.has_rec? && !$ctl_mic[:forget]
            ...
            Interact::clear_area_comment
            print "\e[#{$lines[:comment]}H\e[0m\e[32m"
            Text::do_figlet_unwrapped 'what  you  played ...', 'smblock'
            sleep 0.5
        ...
        if $opts[:auto_replay] && !seq_played_recently
          Interact::clear_area_comment
          print "\e[#{$lines[:comment]}H\e[0m\e[32m"
          Text::do_figlet_unwrapped 'listen ...', 'smblock'
          Players::play_lick_rec_or_holes to_play, oride_l_message2
          seq_played_recently = true
        end
        ...
        to_play[:all_wanted].each_with_index do |wanted, idx|
            ...
            lambda do |*_|
              if idx != idx_refresh_comment_cache || $ctl_mic[:update_comment]
                ...
                comment_cache =
                  case $opts[:comment]
                  when :holes_some
                    Text::largify(to_play[:all_wanted], idx)
                  when :holes_scales
                    holes_with_scales ||= scaleify(to_play[:all_wanted])
                    Text::tabify_colorize($lines[:hint_or_message] - $lines[:comment_tall], holes_with_scales, idx)
                  when :holes_intervals
                    holes_with_intervals ||= intervalify(to_play[:all_wanted], prefer_names: true)
                    Text::tabify_colorize($lines[:hint_or_message] - $lines[:comment_tall], holes_with_intervals, idx)
                  when :holes_inter_semis
                    holes_with_inter_semis ||= intervalify(to_play[:all_wanted], prefer_names: false)
                    Text::tabify_colorize($lines[:hint_or_message] - $lines[:comment_tall], holes_with_inter_semis, idx)
                  when :holes_notes
                    holes_with_notes ||= noteify(to_play[:all_wanted])
                    Text::tabify_colorize($lines[:hint_or_message] - $lines[:comment_tall], holes_with_notes, idx)
                  when :holes_all
                    Text::wrapify_for_comment($lines[:hint_or_message] - $lines[:comment_tall], to_play[:all_wanted], idx)
                  else
                    raise "Internal error: unknown comment style: #{$opts[:comment]}"
                  end
          ...
          break if %i[next back replay replay_menu shift_inter change_partial forget change_lick edit_lick_file change_tags reverse_holes shuffle_holes lick_info toggle_record_user change_num_quiz_replay quiz_hint first_lick].any? {|k| $ctl_mic[k]}
        ...
        # no echo of action
          ...
          $ulrec.ensure_end_rec
          Interact::clear_area_comment
          if %i[holes_scales holes_intervals holes_inter_semis].include?($opts[:comment])
            print "\e[#{$lines[:comment] + 2}H\e[0m\e[32m   again"
          else
            print "\e[#{$lines[:comment]}H\e[0m\e[32m"
            Text::do_figlet_unwrapped 'again', 'smblock'
          end
          sleep 0.3
        else
          sleep 0.3
          Interact::clear_area_comment if %i[holes_all holes_scales holes_intervals holes_inter_semis].include?($opts[:comment])
          # update comment
          cmnt_col = 32
          cmnt_text = if $ctl_mic[:next]
                      ...
                      elsif %i[change_lick edit_lick_file change_tags reverse_holes shift_inter shuffle_holes change_partial change_num_quiz_replay].any? {|k| $ctl_mic[k]}
          ...
          if cmnt_text
            ...
            if %i[holes_scales holes_intervals holes_inter_semis].include?($opts[:comment])
              ...
              Text::do_figlet_unwrapped cmnt_text, 'smblock'
            end
            print "\e[0m"
      ...
      end while ( $ctl_mic[:loop] || $ctl_mic[:forget] ) && %i[back next replay replay_menu shift_inter change_partial change_lick edit_lick_file change_tags reverse_holes shuffle_holes change_num_quiz_replay first_lick].all? {|k| !$ctl_mic[k]}  ## while looping over the same sequence again and again
  ...
  def play_lick_recording_and_handle_kb_plus lick, at_line:, shift_inter:, holes:
    ...
    skipped = Players::play_lick_recording_and_handle_kb(lick, start, length, shift_inter, !!at_line)
  ...
  def select_and_calc_partial all_holes, start_s, length_s
    ...
    if md = $opts[:partial].match(%r{^1/(\d)@(b|x|e)$})
  ...
  def read_tags_and_refresh_licks curr_lick
    ...
    tags_all = Choose::choose_interactive("Choose new value for --tags-all, aka -t (current is #{$opts[:tags_all]}): ", (['.initial'] + all_tags).flatten) do |tag|
      if tag == '.initial'
    ...
    changed = if tags_all == '.initial'
                ...
                $all_licks, $licks, $all_lick_progs = Licks::read_licks(graceful: true)
                true
              elsif tags_all
                $opts[:tags_all] = tags_all
                $all_licks, $licks, $all_lick_progs = Licks::read_licks(graceful: true)
                iter = Choose::choose_interactive('Choose new value for --iterate, aka -i: ',
  ...
  def get_replay_flags
    ...
    answer = Choose::choose_interactive('Choose flags for one replay: ',
                                        ch2desc.keys) do |tag|
      ch2desc[tag]
    end
    Interact::clear_area_comment
  ...
  def read_and_set_partial
    ...
    input = Interact::gets_with_cursor.chomp
    old = $opts[:partial]
    ...
    begin
      ...
      Interact::report_condition_wait_key e
      $opts[:partial] = old
    end
  ...
  def read_and_set_num_quiz_replay
    ...
    puts
    print "\e[0mYour input: "
    input = Interact::gets_with_cursor
    new = input.to_i
  ...
  def print_comment_adhoc holes, quiz_and_after: false
    ...
    lines = if $opts[:comment] == :holes_scales
              ...
              Text::tabify_colorize($lines[:hint_or_message] - $lines[:comment_tall],
  ...
  def show_lick_info lick
    ...
    puts '        Desc:  ' + lick[:desc]
    puts Text::wrap_words('        Tags:  ', lick[:tags])
    if $opts[:lick_prog]
  ...
  def puts_names_of_licks maxnum
    ...
    puts Text::wrap_words('   All licks,  ', ["#{$licks.length} in total:  " + names[0]] + names[1..-1], ',')
  end
...
class PlayController < Struct.new(:all_wanted, :all_wanted_befores, :lick, :lick_idx, :lick_idx_befores, :lick_hints, :shift_inter, :show_in_play)
  ...
  def read_name_change_lick curr_lick
    ...
    input = Choose::choose_interactive("Please choose lick (current is #{curr_lick[:name]}): ", choices.flatten) do |lname|
    ...
    set_lick_and_others_from_idx(new_idx)
    self[:shift_inter] = 0
  end
  ...
  def back_one_sequence
    if self[:lick]
      ...
      if self[:lick_idx_befores].length == 0
        ...
        set_lick_and_others_from_idx(self[:lick_idx_befores].pop)
      end
    else
  ...
  def edit_lick
    if self[:lick][:tags].include?('not-from-lickfile')
      Interact::report_condition_wait_key "This lick (#{self[:lick][:name]}) is not from lick file; cannot edit here.\nRather switch to mode 'listen' to edit.", :info
    elsif Util::edit_file($lick_file, self[:lick][:lno])
      if self[:lick_idx] && Licks::refresh_licks
        # rare edge case, e.g. when we remove tag 'fav' from last lick
        unless $licks[self[:lick_idx]]
          self[:lick_idx_befores] = Array.new
          self[:lick_idx] = 0
        end
  ...
  def change_tags
    start_new_iteration = ModeLicks::read_tags_and_refresh_licks(self[:lick])
  ...
  def read_and_shift_inter
    ...
    answer = Choose::choose_interactive('Choose an interval to shift (curr. %+dst):' % self[:shift_inter], choices_desc.keys) do |inter|
      choices_desc[inter]
    end&.strip
    ...
    if answer
      ...
      if num_holes_playable[shift] > 0
        ...
        set_all_wanted(self[:lick][:holes].map do |hole|
                         if Theory::musical_event?(hole)
                           hole
                         else
        ...
        if shift == 0
          self[:shift_inter] = 0
          set_all_wanted self[:lick][:holes]
  ...
  def continue_with_cycle
    set_lick_idx(self[:lick_idx] + 1)
    return unless self[:lick_idx] >= $licks.length

    self[:lick_idx] = 0
    Interact::ctl_response 'Next cycle'
  end

  def choose_random_lick
    if self[:lick_idx_befores].length > 0
      # avoid playing the same lick twice in a row
      set_lick_idx((self[:lick_idx] + 1 + rand($licks.length - 1)) % $licks.length)
    else
      set_lick_idx rand($licks.length)
    end
  end

  def back_to_first_lick
    set_lick_idx(0)
    set_lick_and_others_from_idx(0)
  end
