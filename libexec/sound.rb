...
module Sound
  ...
  def this_or_equiv template, note, endings = ['']
    endings.each do |ending|
      Theory::notes_equiv(note).each do |eq|
      ...
      err(
        if IO.select([ppl_err], nil, nil, 2)
          "Command terminated unexpectedly: #{cmd}\n" +
            ...
            ppl_err.sysread(65_536).lines.map {|l| " >> #{l}"}.join
        else
    ...
    loop do
      ...
      Timeout.timeout(10) do
        ...
        20.times do
          ...
          until (line = ppl_out.gets)
            sleep 0.5
            no_gets += 1
            next unless no_gets > 10
          ...
          # output of pipeline.
          iters += 1
          next unless iters == $jitter_check_after_iters
          ...
          # check for jitter (now and then)
        end
      end
    rescue Timeout::Error
      err "No output from: #{cmd}"
      # check for timeout in gets
    end  # one batch of frequencies after the other
  end

  def get_pipeline_cmd(what, wav_from)
    err "Internal error: #{what}" unless %i[pv sox].include?(what)
    #
  ...
  def play_hole_or_note_and_collect_kb hon, duration
    ...
    wait_thr = Thread.new do
      if $testing
        ...
        Util::sys "play -q -n --norm=#{$vol.to_i} synth #{duration} sawtooth %#{Theory::note2semi(note)}", $sox_fail_however
      end
    end
    begin
      sleep 0.1
  ...
  def inspect_recorded hole, file
    ...
    if too_low || too_high
      puts "\n\e[0;101mWARNING:\e[0m\n\e[33mThe frequency recorded for hole  -->   \e[93m#{hole}\e[33m   <--  (note #{$harp[hole][:note]}, semi #{semi})\nis too different from ET tuning:"
...
class UserLickRecording
  ...
  def process_rec
    ...
    @first_hole_good_at = nil
    @has_rec = true
    @duration = Sound::sox_query(@file_trimmed, 'Length')
  end
