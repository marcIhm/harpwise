...
def do_quiz to_handle
  ...
  err "'harpwise quiz #{$extra}' does not take any arguments, these cannot be handled: #{to_handle}" if $extra != 'recall' && to_handle.length > 0
  ...
  if $extra == 'replay'
    err "'harpwise quiz recall' requires exactly one integer argument, not: #{to_handle}" unless to_handle.length == 1 && to_handle[0].match(/^\d+$/)
    $num_quiz = to_handle[0].to_i
  end
