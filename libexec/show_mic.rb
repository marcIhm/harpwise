  ...
  def enter_loop lambda_mission, lambda_good_done_was_good, lambda_skip,
    ...
    loop do   # over each new frequency from pipeline, until done or skip
      ...
      if $first_round_ever_get_hole
        ...
        if $mode == :listen
          spl = Text::animate_splash_line(true)
          $msgbuf.print spl, 2, 2
          Interact::check_screen
          if $term_width > 1.5 * $conf[:term_min_width] || $term_height > 1.5 * $conf[:term_min_height]
            $msgbuf.print [spl,
                           "This terminal [#{$term_width}, #{$term_height}] is much larger than needed [#{$conf[:term_min_width]}, #{$conf[:term_min_height]}] ...",
                           '... consider enlarging font for better readability'], 5, 5, truncate: false, wrap: false
          end
        end
      end
        ...
        if hole_journal &&
          ...
          if tntf - hole_journal_since > $journal_minimum_duration
            ...
            unless Theory::musical_event?($journal[-1])
              ...
              $msgbuf.print("#{ModeListen::journal_length} holes", 2, 5, :journal) if $opts[:comment] == :journal
            end
      ...
      if $ctl_mic[:player_details]
        ...
        if $players.stream_current
          ...
          ModePrint::print_player($players.structured[$players.stream_current])
