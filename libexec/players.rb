...
module Players
  ...
  def play_lick_recording_and_handle_kb lick, start, length, shift_inter, scroll_allowed
    ...
    tempo = 1.0
    imm_ctrls_again = %i[replay slower faster vol_up vol_down]
    ...
    begin
      ...
      begin
        ...
        if $ctl_rec[:pause_continue]
          ...
          pplayer.pause
          Util::display_kb_help 'the recording of a lick', scroll_allowed,
  ...
  def play_recording_and_handle_kb recording, timed_comments = nil, scroll_allowed = true
    imm_ctrls_again = %i[replay vol_up vol_down]
    ...
    begin
      ...
      cmd = if recording.is_a?(Array)
              "play --norm=#{$vol.to_i} -q -V1 --combine mix #{recording.join(' ')}".strip
            else
              "play --norm=#{$vol.to_i} -q -V1 #{recording}".strip
            end
      IO.write($testing_log, cmd + "\n", mode: 'a') if $testing
      if $testing_what == :player
        cmd = 'sleep 100'
      ...
      begin
        sleep 0.1
        Interact::handle_kb_play_recording
        if $ctl_rec[:pause_continue]
          $ctl_rec[:pause_continue] = false
          pplayer.pause
          Interact::space_to_cont
          ...
          pplayer.pause
          Util::display_kb_help 'a recording', scroll_allowed,
                                "         SPACE: pause/continue\n" +
                                "  TAB,+,RETURN: skip to end           -: back to start\n" +
        ...
        if timed_comments && timed_comments.length > 0 && (pplayer.time_played > timed_comments[0][0])
  ...
  def play_interactive_interval semi1, semi2
    ...
    loop do
      ...
      begin
        ...
        Interact::handle_kb_play_inter
        sleep 0.1
      end while pplayer.alive?

      # handle kb
      next unless $ctl_inter[:any] || $ctl_inter[:invalid]
      ...
      if $ctl_inter[:pause_continue]
        ...
        Util::display_kb_help 'an interval', true,
  ...
  def play_interactive_chord semis, args_orig
    ...
    loop do
      ...
      begin
        ...
        Interact::handle_kb_play_chord
        sleep 0.1
      end while pplayer.alive?

      # handle_kb
      next unless $ctl_chord[:any]
      ...
      if $ctl_chord[:pause_continue]
        ...
        single = if single
                   Util::rotate_among(single, :up, args_orig)
                 else
                   args_orig[0]
                 end
        new_sound = true
        ...
        pplayer.pause
        Util::display_kb_help 'a chord', true,
                              "  SPACE: pause/continue  ESC,x,q: quit\n" +
                              "      w: change waveform       W: change waveform back\n" +
                              "      g: decrease time gap     G: increase\n" +
      ...
      [holes, :delay].flatten.zip(notes, abs_semis, rel_semis).each do |ho, no, as, rs|
        ...
        begin
          ...
          if $ctl_prog[:pause_continue]
            ...
            Util::display_kb_help 'a semitone progression', true,
                                  "    SPACE: pause/continue\n" +
  ...
  def play_lick_holes_and_handle_kb all_holes, at_line: nil, scroll_allowed: false, lick: nil, with_head: false, hide_holes: false
    ...
    begin
      ...
      [holes, '(0.5)'].flatten.each_cons(2).each_with_index do |(hole, hole_next), _idx|
        ...
        print( Theory::musical_event?(hole) ? "\e[2m#{hole_disp}\e[2m " : "\e[0m#{hole_disp}\e[2m " )
        ...
        if $ctl_hole[:show_help]
          Util::display_kb_help 'the holes of a lick', scroll_allowed,
                                '       SPACE: pause/continue' +
                                if $ctl_lk_hl[:can_star_unstar]
                                  "          *,/: star,unstar lick\n"
                                else
